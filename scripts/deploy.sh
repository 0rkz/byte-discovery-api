#!/usr/bin/env sh
# Enforced deploy for byte-discovery. The service runs a BUILD ARTIFACT under
...
#
# byte-discovery's ExecStart is `node dist/index.js`, but index.js is
# only the ENTRY POINT: it `require`s ./lib/config and ./lib/feeds.
...
# checks it against what byte-discovery actually runs. Repoint that unit's ExecStart, or
...
# use the ABSOLUTE form, so normalising byte-discovery to the house majority would have
...
echo "[deploy] guard passed — restarting byte-discovery"
systemctl --user restart byte-discovery
sleep 3
...
systemctl --user is-active --quiet byte-discovery || {
  echo "[deploy] ABORT: byte-discovery did not come back active after restart"
  systemctl --user --no-pager status byte-discovery | head -20
  exit 1
}
systemctl --user --no-pager status byte-discovery | head -5
echo "✅ byte-discovery deployed (committed source verified, artifact rebuilt, unit active)"
